...
def job_statistiche() -> None:
    ...
    for tr in table.select("tbody tr"):
        ...
        if not a:
            continue
        m = re.search(r"/serie-a/squadre/([a-z0-9\-]+)/([a-z0-9\-\.]+)/(\d+)", a.get("href", ""))
...
def job_calendario() -> None:
    ...
    # pattern robusto sui link squadra: /serie-a/squadre/<slug>
    slugs = re.findall(r"/serie-a/squadre/([a-z0-9\-]+)", html)
    # dedup mantenendo l'ordine: la pagina elenca le partite in sequenza casa,ospite
